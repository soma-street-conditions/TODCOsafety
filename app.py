...
# 7. Helper: VERINT IMAGE CRACKER (Debug Mode)
def fetch_verint_image(wrapper_url, case_id, debug_mode=False):
    ...
    try:
        ...
        html = r_page.text
        if debug_mode: logs.append("✅ Step 1 OK: Page Loaded")

        # STEP 2: EXTRACT TOKENS
        # Regex matches the pattern we saw in the probe/HTML
        formref_match = re.search(r'"formref"\s*:\s*"([^"]+)"', html)
        if not formref_match:
            if debug_mode: logs.append("❌ Step 2 Failed: formref not found in HTML")
            return None, logs
        ...
        if debug_mode: logs.append(f"✅ Step 2 OK: Found formref ({formref}) and CSRF ({bool(csrf_token)})")

        # STEP 3: API CALL (GET FILENAMES)
        ...
        filename_str = ""
        # Check specific path we saw in inspection
        if 'data' in files_data and 'formdata_filenames' in files_data['data']:
            filename_str = files_data['data']['formdata_filenames']
            
        if not filename_str:
            if debug_mode: logs.append(f"❌ Step 3 Failed: Filename list empty. JSON: {str(files_data)[:50]}")
            return None, logs
            
        raw_files = filename_str.split(';')
        if debug_mode: logs.append(f"✅ Step 3 OK: Found files {raw_files}")
        ...
        if not target_filename: 
             if debug_mode: logs.append("❌ Step 4 Failed: No non-map image found")
             return None, logs
        ...
        if r_image.status_code == 200:
            if debug_mode: logs.append(f"✅ Step 5 OK: Downloaded {len(r_image.content)} bytes")
        ...
        if media_content:
            ...
            with cols[col_index]:
                with st.container(border=True):
                    ...
                    # PRINT DEBUG LOGS IF FAILED
                    if show_debug and logs and media_type == "broken":
                        st.error("\n".join(logs))
                    elif show_debug and logs and media_type == "bytes":
                        st.success("Image Cracked Successfully")
                    ...
                    # METADATA
                    date_str = pd.to_datetime(row['requested_datetime']).strftime('%b %d, %I:%M %p') if 'requested_datetime' in row else "?"
